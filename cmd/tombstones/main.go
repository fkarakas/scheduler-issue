...
func produceMessages(size int, tombstone bool) {
	...
	for i := 1; i <= size; i++ {
		key := []byte(uuid.NewMD5(uuid.NameSpaceOID, []byte(itoa(i))).String())
		msg := &kafka.Message{
		...
		if !tombstone {
			msg.Value = []byte(fmt.Sprintf("message payload for key: %s", string(key)))
		}
		p.Produce(msg, nil)
	}
...
func consumeMessages(size int) chan *kafka.Message {
...
func (s Store) InsertMessage(msg *kafka.Message) error {
	...
	_, err := s.db.Exec("insert into message(key, timestamp, headers, partition, offset, value) values(?, ?, ?, ?, ?, ?);",
		string(msg.Key), msg.Timestamp.Unix(), headers, msg.TopicPartition.Partition, msg.TopicPartition.Offset, value)
	if err != nil {
		return err
	}
	return nil
...
func main() {
	CreateTopic()
	produceMessages(max, false)
	produceMessages(max, true)
	...
	index := 0
	for m := range consumeMessages(2 * max) {
		index++
		fmt.Printf("received message: key=%s partition=%v index=%v\n", m.Key, m.TopicPartition.Partition, index)
